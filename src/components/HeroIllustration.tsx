/**
 * Inline SVG hero illustration with animated screen content.
 * Based on undraw_programming, recolored to site palette.
 * Laptop code lines and monitor UI scroll continuously.
 */
export default function HeroIllustration({
  ...
  /* Code‑line row generator for the laptop screen.
     Each row is a set of short + long rects mimicking syntax‑highlighted code. */
  const codeRow = (yOffset: number, indent: number) => (
    <g key={yOffset}>
      <rect
        width="6.833"
        height="3.576"
  ...
  /* 8 rows of code, each ~8px apart, alternating indent depth */
  const codeRows = [0, 1, 2, 3, 4, 3, 2, 1].map((indent, i) =>
    codeRow(845.5 + i * 8.07, indent * 10)
  );

  /* Monitor UI rows (wider, simulating a dashboard/report) */
  const uiRow = (yOffset: number, w1: number, w2: number) => (
    <g key={yOffset}>
      <rect
        width={w1}
        height="4"
  ...
  return (
    ...
    >
      <defs>
        {/* Clip to laptop screen area */}
        <clipPath id="laptop-clip">
          <rect x="536" y="825" width="152" height="100" />
        </clipPath>
        {/* Clip to main monitor screen area */}
        <clipPath id="monitor-clip">
          <rect x="63" y="645" width="325" height="215" />
        </clipPath>
      </defs>
      ...
      <style>{`
        @keyframes scroll-code {
          ...
          100% { transform: translateY(-64.56px); }
        }
        @keyframes scroll-ui {
          0% { transform: translateY(0); }
          100% { transform: translateY(-84px); }
        }
        .scroll-code { animation: scroll-code 6s linear infinite; }
        .scroll-ui { animation: scroll-ui 10s linear infinite; }
      `}</style>

      <g transform="translate(-28.583 -435)">
        {/* ── Background building / city ── */}
        <path
        ...
        <path
          ...
          fill="#2a2a28"
        />
        <rect width="4.797" height="2.878" rx="0.488" transform="translate(542.16 932.354) rotate(180)" fill="#141413" />
        <rect width="4.797" height="2.878" rx="0.488" transform="translate(542.274 937.151) rotate(180)" fill="#141413" />
        <path d="M.559,0H37.82a.559.559,0,0,1,.559.559v1.76a.559.559,0,0,1-.559.559H.559A.559.559,0,0,1,0,2.319V.559A.559.559,0,0,1,.559,0Z" transform="translate(640.14 942.909) rotate(180)" fill="#141413" />

        {/* ── Laptop screen code lines (animated) ── */}
        <g clipPath="url(#laptop-clip)">
          <g className="scroll-code">
            {codeRows}
            {/* Duplicate for seamless loop */}
            <g transform="translate(0, 64.56)">{codeRows}</g>
          </g>
        </g>
        ...
        />
        {/* Browser dots */}
        <ellipse cx="6.815" cy="6.967" rx="6.815" ry="6.967" transform="translate(93.317 648.321)" fill="#141413" />
        <ellipse cx="6.815" cy="6.967" rx="6.815" ry="6.967" transform="translate(116.861 648.321)" fill="#141413" />
        <ellipse cx="6.815" cy="6.967" rx="6.815" ry="6.967" transform="translate(140.405 648.321)" fill="#141413" />
        {/* Toolbar lines */}
        <path d="M550.486,351.622H533.763a1.266,1.266,0,0,0,0,2.531h16.722a1.266,1.266,0,1,0,0-2.531Z" transform="translate(-191.998 298.422)" fill="#141413" />
        <path d="M550.489,355.772H533.767a1.266,1.266,0,1,0,0,2.532h16.722a1.266,1.266,0,1,0,0-2.531Z" transform="translate(-192.002 299.025)" fill="#141413" />
        <path d="M550.486,359.922H533.763a1.266,1.266,0,0,0,0,2.531h16.722a1.266,1.266,0,1,0,0-2.531Z" transform="translate(-191.998 299.629)" fill="#141413" />

        {/* ── Monitor screen UI content (animated) ── */}
        <g clipPath="url(#monitor-clip)">
          <g className="scroll-ui">
            {uiRows}
            {/* Duplicate for seamless loop */}
            <g transform="translate(0, 168)">{uiRows}</g>
          </g>
        </g>

        <path d="M0,0H138.582l2.729,9.163H0Z" transform="translate(544.477 928.654)" fill="#141413" />
